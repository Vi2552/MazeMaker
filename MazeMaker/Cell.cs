...
namespace MazeMaker
{
    ...
    class Cell
    {
        ...
        internal void Highlight(Graphics graphics)
        {
            ...
            graphics.FillRectangle(Brushes.GreenYellow, x, y, Size, Size);

            if (walls[0])
            {
                graphics.DrawLine(Pens.White, x, y, x + Size, y); //top
            }
            if (walls[1])
            {
                graphics.DrawLine(Pens.White, x + Size, y, x + Size, y + Size); // right
            }
            if (walls[2])
            {
                graphics.DrawLine(Pens.White, x + Size, y + Size, x, y + Size); // bottom
            }
            if (walls[3])
            {
